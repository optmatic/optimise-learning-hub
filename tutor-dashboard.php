    ...
    <div class="row">
        ...
        <div class="tab-content" id="myTabContent" style="padding-top: 20px;">
                ...
                <div class="tab-pane fade" id="classroom-url" role="tabpanel" aria-labelledby="classroom-url-tab">
                    ...
                    // Fallback to old classroom_url
                    $classroom_url = get_user_meta($user_id, 'classroom_url', true);
                    
                    if (!empty($math_classroom) || !empty($english_classroom) || !empty($custom_classroom_url) || !empty($classroom_url)) {
                        echo '<p>Please see the "Your Classroom URLs" tab for all your classroom links.</p>';
                    } else {
                    ...
                    if (!empty($math_classroom) || !empty($english_classroom) || !empty($custom_classroom_url) || !empty($old_classroom_url)) {
                        ...
                        if (!empty($math_classroom)) {
                            echo '<li><strong>Mathematics:</strong> <a href="' . esc_url($math_classroom) . '" target="_blank">' . esc_html($math_classroom) . '</a></li>';
                        }
                        
                        if (!empty($english_classroom)) {
                            echo '<li><strong>English:</strong> <a href="' . esc_url($english_classroom) . '" target="_blank">' . esc_html($english_classroom) . '</a></li>';
                        }
                        
                        if (!empty($custom_classroom_url) && !empty($custom_classroom_name)) {
                            echo '<li><strong>' . esc_html($custom_classroom_name) . ':</strong> <a href="' . esc_url($custom_classroom_url) . '" target="_blank">' . esc_html($custom_classroom_url) . '</a></li>';
                        } elseif (!empty($custom_classroom_url)) {
                            echo '<li><strong>Custom Classroom:</strong> <a href="' . esc_url($custom_classroom_url) . '" target="_blank">' . esc_html($custom_classroom_url) . '</a></li>';
                        }
                        
                        // Display old classroom URL if it exists
                        if (!empty($old_classroom_url)) {
                            echo '<li><strong>Classroom:</strong> <a href="' . esc_url($old_classroom_url) . '" target="_blank">' . esc_html($old_classroom_url) . '</a></li>';
                        }
